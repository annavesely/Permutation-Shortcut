...
#' the lowest and the highest. The sizes are examined in increasing order.
#'
...
#' \code{indecisive} (vector of sizes with indecisive outcome). The sizes in \code{indecisive}
#' are intended as the number of indices not under testing that are added to
#' the original subset.
#'
#' @author
#'
...
incr <- function(Ds, Dc, m=ncol(Dc), alpha=0.05){
  L <- Ds
  U <- Ds
  cond <- TRUE
  ind <- 1:(m-1)
