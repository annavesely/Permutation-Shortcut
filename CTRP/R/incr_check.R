#' @title Increasing check
#'
#' @description Internal function. It checks the bounds for different superset sizes,
#' considered in increasing order from 1 up to a given maximum.
#'
#' @param Ds numerical vector, sum of the centered test statistics of the indices under testing
#' @param Dc matrix of the centered test statistics of the indices not under testing
#' @param m number of columns of \code{Dc}
...
incr_check <- function(Ds, Dc, m=ncol(Dc), alpha=0.05){
  L <- Ds
  U <- Ds
  cond <- TRUE
  ind <- 1:(m-1)
